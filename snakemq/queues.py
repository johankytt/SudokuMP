...
class Queue(object):
    ...
    def load_persistent_data(self):
        self.queue[:] = self.manager.storage.get_items(self._name)
    ...
    def collect_garbage(self):
        ...
        for item in deleted_items:
            self.manager.on_item_drop(self._name, item.uuid)
    ...
    def push(self, item):
        ...
        if to_store and ((item.ttl is None) or (item.ttl > 0)):
            ...
            self.manager.storage.push(self._name, item)
